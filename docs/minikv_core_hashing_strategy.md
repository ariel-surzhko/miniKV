...
miniKV will treat the hash-table index as two possible approaches:
...
if one chain is too long while α is fine:
...
This is not the default Redis-like mode.
It is a second option.
...
```text
Dynamic hashing: expected O(1 + α)
...
→ expected O(1 + α)
→ expected O(1) only when α is bounded
```
